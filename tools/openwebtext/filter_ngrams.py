...
def check_and_clean_text(args, words, ngrams, text, start_position, text_buf_ngram_free, text_buf, local_ngram):
...
def compute_ngram_freq_sorted(args, ngrams):
    ...
    print(
        " Entities in ngrams {} min_ngram_size {} max_ngram_size {}".format(
            len(ngrams), ngrams_freq_sorted[0][0], ngrams_freq_sorted[len(ngrams_freq_sorted) - 1][0]
        ),
        flush=True,
    )
...
def get_ngrams_below_threshold(args, ngrams, ngrams_below_threshold, dedup_file, dedup_key, ngrams_freq_sorted):
    ...
    print(" Time taken to compute statistics {:.2f} seconds".format(time.time() - start_time), flush=True)
    pool.close()
    pool.join()
...
def clean_ngrams_below_threshold(args, ngrams_below_threshold, dedup_file, dedup_key):
    ...
    free_ngram_clean_partial = partial(
        free_ngram, args=args, key=dedup_key, ngrams=ngrams_below_threshold, ngrams_freq_sorted=ngrams_freq_sorted
    )
    ...
    print(
        " [final]> processed {} documents in {:.2f} seconds ...".format(counter, time.time() - start_time), flush=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambada-path", type=str, default=None, help="Only Lambada task needs the path")
    parser.add_argument(
        "--dedup-dataset",
        nargs="*",
        default=None,
    ...
    parser.add_argument("--output", type=str, default=None, help="Output file name to save dedup dataset")
    ...
    parser.add_argument("--filter-text-char-len", type=int, default=200, help="Remove any text below this length.")
    parser.add_argument("--key-threshold", type=int, default=10, help="Number of keys to consider as threshold")
    ...
    parser.add_argument("--splits-count", type=int, default=10, help="Remove any documents more than this many splits")
    parser.add_argument("--remove-char-each-side", type=int, default=200, help="Maximum size of ngram to use.")
    ...
    if args.load_dictionary is None:
        ...
        get_ngrams_below_threshold(args, ngrams, ngrams_below_threshold, dedup_file, dedup_key, ngrams_freq_sorted)
