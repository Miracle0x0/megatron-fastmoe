        ...
        if os.getpid() % num_bins == 0 and bucket_local % 100000 == 0:
            print(
                "Counter {}, progress {:.2f} time {:.2f}".format(
                    bucket_local, float(bucket_local) / float(len(each_bin)), time.time() - start_time_local
                ),
                flush=True,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--heuristic-iter", type=int, default=1, help="Number of iterations to run the heuristics" ": use -1 for exact"
    )
    ...
    parser.add_argument(
        "--jaccard-parallel", action="store_true", help="Use this to process large number of documents."
    )
