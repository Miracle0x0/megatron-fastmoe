...
def score_and_return_on_first_stage(model, tokens, lengths):
    ...
    if mpu.is_pipeline_last_stage():
        output_log_probs = torch.empty(output_log_probs_size, dtype=torch.float32, device=torch.cuda.current_device())
            ...
            if mpu.is_pipeline_last_stage():
                ...
                if return_output_log_probs:
                    ...
                    if return_output_log_probs:
                        ...
                        # the token which we selected in the current logits,
                        # so shift by 1.
                        indices = torch.unsqueeze(tokens[:, (prev_context_length + 1) : (context_length + 1)], 2)
...
def _build_attention_mask_and_position_ids(tokens):
    ...
    attention_mask, _, position_ids = get_ltor_masks_and_position_ids(
        data=tokens, eod_token=None, reset_position_ids=False, reset_attention_mask=False, eod_mask_loss=False
    )
