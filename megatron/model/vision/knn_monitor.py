...
def compute_feature_bank(model):
    ...
    feature_banks = [torch.zeros_like(feature_bank) for i in range(mpu.get_data_parallel_world_size())]
    ...
    feature_labels = [torch.zeros_like(feature_label) for i in range(mpu.get_data_parallel_world_size())]
