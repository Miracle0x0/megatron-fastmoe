...
def get_student_backbone_and_num_features(pre_process=True, post_process=True):
    ...
    if args.vision_backbone_type == "vit":
        student = VitBackbone(
            pre_process=pre_process, post_process=post_process, drop_path_rate=0.1, single_token_output=True
        )
...
class DINOPretrainModel(MegatronModule):
    def __init__(self, pre_process=True, post_process=True):
        ...
        self.student = MultiCropWrapper(
            student_backbone, DINOHead(num_features, self.out_dim, norm_last_layer=args.dino_norm_last_layer)
        )
