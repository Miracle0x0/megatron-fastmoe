...
def get_model_provider(only_query_model=False, only_context_model=False, biencoder_shared_query_context_model=False):
...
class BiEncoderModel(MegatronModule):
    ...
    def forward(
        self, query_tokens, query_attention_mask, query_types, context_tokens, context_attention_mask, context_types
    ):
        """Run a forward pass for each of the models and
        return the respective embeddings."""

        if self.use_query_model:
            query_logits = self.embed_text(self.query_model, query_tokens, query_attention_mask, query_types)
        else:
            raise ValueError("Cannot embed query without the query model.")
        if self.use_context_model:
            context_logits = self.embed_text(self.context_model, context_tokens, context_attention_mask, context_types)
        else:
    ...
    def init_state_dict_from_bert(self):
        ...
        if mpu.get_data_parallel_rank() == 0:
            print("global rank {} is loading BERT checkpoint {}".format(torch.distributed.get_rank(), checkpoint_name))
        ...
        if self.biencoder_shared_query_context_model:
            ...
            if self.use_query_model:
                ...
                if self.biencoder_projection_dim > 0:
                    query_proj_state_dict = self.state_dict_for_save_checkpoint()[self._query_key]["projection_enc"]
...
class PretrainedBertModel(MegatronModule):
    ...
    def __init__(self, num_tokentypes=2, parallel_output=True, pre_process=True, post_process=True):
        ...
        if args.biencoder_projection_dim > 0:
            self.projection_enc = get_linear_layer(args.hidden_size, args.biencoder_projection_dim, init_method)
    ...
    def state_dict_for_save_checkpoint(self, destination=None, prefix="", keep_vars=False):
        ...
        if self.biencoder_projection_dim > 0:
            state_dict_[self._projection_enc_key] = self.projection_enc.state_dict(destination, prefix, keep_vars)
