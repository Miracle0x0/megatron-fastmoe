...
def forward_step(
    forward_step_func, data_iterator, model, input_tensor, forward_data_store, collect_non_loss_data=False
):
    ...
    # downstream as well.
    if mpu.is_pipeline_stage_after_split() and args.model_type == ModelType.encoder_and_decoder:
...
def forward_backward_no_pipelining(
    forward_step_func, data_iterator, model, optimizer, timers, forward_only, collect_non_loss_data=False
):
    ...
    with context_handler():
        for i in range(get_num_microbatches() - 1):
            output_tensor, bal_loss = forward_step(
                forward_step_func, data_iterator, model, input_tensor, forward_data_store, collect_non_loss_data
            )
            if not forward_only:
    ...
    for k in range(num_warmup_microbatches):
        ...
        if k == (num_warmup_microbatches - 1) and not forward_only and not all_warmup_microbatches:
            ...
            input_tensor = p2p_communication.send_forward_recv_forward(
                output_tensor, recv_prev=recv_prev, tensor_shape=tensor_shape, timers=timers
            )
        ...
        for k in range(num_microbatches_remaining, num_microbatches):
            ...
            output_tensor_grads[next_backward_model_chunk_id].append(
                p2p_communication.send_backward_recv_backward(
                    input_tensor_grad, recv_next=recv_next, tensor_shape=tensor_shape, timers=timers
                )
            )
...
def recv_forward(tensor_shapes, timers):
    ...
    for tensor_shape in tensor_shapes:
        if tensor_shape is None:
            ...
            input_tensors.append(p2p_communication.recv_forward(tensor_shape, timers=timers))
...
def recv_backward(tensor_shapes, timers):
    ...
    for tensor_shape in tensor_shapes:
        if tensor_shape is None:
            ...
            output_tensor_grads.append(p2p_communication.recv_backward(tensor_shape, timers=timers))
...
def send_forward_recv_backward(output_tensors, tensor_shapes, timers):
    ...
    for output_tensor, tensor_shape in zip(output_tensors, tensor_shapes):
        if tensor_shape is None:
            output_tensor_grads.append(None)
            continue
        output_tensor_grad = p2p_communication.send_forward_recv_backward(output_tensor, tensor_shape, timers=timers)
...
def send_backward_recv_forward(input_tensor_grads, tensor_shapes, timers):
    ...
    for input_tensor_grad, tensor_shape in zip(input_tensor_grads, tensor_shapes):
        if tensor_shape is None:
            input_tensors.append(None)
            continue
        input_tensor = p2p_communication.send_backward_recv_forward(input_tensor_grad, tensor_shape, timers=timers)
...
def forward_backward_pipelining_without_interleaving(
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            output_tensor_grad = send_forward_recv_backward(output_tensor, send_tensor_shapes, timers=timers)
            ...
            input_tensor_grad = backward_step(optimizer, input_tensor, output_tensor, output_tensor_grad, bal_loss)

            if last_iteration:
                input_tensor = None
                send_backward(input_tensor_grad, recv_tensor_shapes, timers=timers)
            else:
                input_tensor = send_backward_recv_forward(input_tensor_grad, recv_tensor_shapes, timers=timers)
