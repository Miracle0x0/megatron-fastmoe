...
def parse_args(extra_args_provider=None, defaults={}, ignore_unknown_args=False):
    ...
    args.transformer_pipeline_model_parallel_size = (
        args.pipeline_model_parallel_size - 1 if args.standalone_embedding_stage else args.pipeline_model_parallel_size
    )
    # Checks.
    ...
    assert args.world_size % model_parallel_size == 0, (
        ...
        "size ({})".format(args.world_size, args.tensor_model_parallel_size, args.pipeline_model_parallel_size)
    )
...
def _add_network_size_args(parser):
    ...
    group.add_argument(
        "--onnx-safe", type=bool, required=False, help="Use workarounds for known problems with " "Torch ONNX exporter"
    )
    group.add_argument(
        "--bert-no-binary-head", action="store_false", help="Disable BERT binary head.", dest="bert_binary_head"
    )
    group.add_argument(
        "--num-experts", type=int, default=None, help="Number of Experts in Switch Transformer (None means no Switch)"
    )
    return parser
...
def _add_logging_args(parser):
    ...
    group.add_argument(
        "--log-batch-size-to-tensorboard", action="store_true", help="If set, write batch-size to tensorboard."
    )
    group.add_argument(
    ...
    group.add_argument(
        "--log-world-size-to-tensorboard", action="store_true", help="Enable world size logging to tensorboard."
    )
...
def _add_regularization_args(parser):
    ...
    group.add_argument(
        "--start-weight-decay", type=float, help="Initial weight decay coefficient for L2 regularization."
    )
    group.add_argument(
        "--end-weight-decay", type=float, help="End of run weight decay coefficient for L2 regularization."
    )
    group.add_argument(
...
def _add_training_args(parser):
    ...
    group.add_argument(
        "--exit-duration-in-mins", type=int, default=None, help="Exit the program after this many minutes."
    )
    group.add_argument(
        "--exit-signal-handler",
        action="store_true",
    ...
    group.add_argument(
        "--no-bias-gelu-fusion", action="store_false", help="Disable bias and gelu fusion.", dest="bias_gelu_fusion"
    )
    group.add_argument(
...
def _add_initialization_args(parser):
    ...
    group.add_argument(
        "--seed", type=int, default=1234, help="Random seed used for python, numpy, " "pytorch, and cuda."
    )
    group.add_argument(
    ...
    group.add_argument(
        "--init-method-xavier-uniform", action="store_true", help="Enable Xavier uniform parameter initialization"
    )
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument(
        "--lr-warmup-iters", type=int, default=0, help="number of iterations to linearly warmup " "learning rate over."
    )
    group.add_argument(
        "--lr-warmup-samples", type=int, default=0, help="number of samples to linearly warmup " "learning rate over."
    )
    group.add_argument(
        "--warmup",
        type=int,
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument(
        "--no-load-optim", action="store_true", default=None, help="Do not load optimizer when loading checkpoint."
    )
    group.add_argument(
        "--no-load-rng", action="store_true", default=None, help="Do not load rng state when loading checkpoint."
    )
    group.add_argument(
        "--finetune",
        action="store_true",
...
def _add_mixed_precision_args(parser):
    ...
    group.add_argument(
        "--initial-loss-scale", type=float, default=2**32, help="Initial loss-scale for dynamic loss scaling."
    )
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        "--pipeline-model-parallel-size", type=int, default=1, help="Degree of pipeline model parallelism."
    )
    group.add_argument(
...
def _add_data_args(parser):
    ...
    group.add_argument(
        "--decoder-seq-length", type=int, default=None, help="Maximum decoder sequence length to process."
    )
    group.add_argument(
    ...
    group.add_argument("--short-seq-prob", type=float, default=0.1, help="Probability of producing a short sequence.")
...
def _add_biencoder_args(parser):
    ...
    group.add_argument(
        "--embedding-path", type=str, default=None, help="Where to save/load Open-Retrieval Embedding" " data to/from"
    )

    # indexer
    group.add_argument(
        "--indexer-batch-size", type=int, default=128, help="How large of batches to use when doing indexing " "jobs"
    )
    group.add_argument(
...
def _add_vision_args(parser):
    ...
    group.add_argument("--img-h", type=int, default=224, help="Image height for vision classification task")
    group.add_argument("--img-w", type=int, default=224, help="Image height for vision classification task")
    ...
    group.add_argument("--classes-fraction", type=float, default=1.0, help="training with fraction of classes.")
    group.add_argument(
        "--data-per-class-fraction", type=float, default=1.0, help="training with fraction of data per class."
    )
    group.add_argument("--no-data-sharding", action="store_false", help="Disable data sharding.", dest="data_sharding")
    group.add_argument(
        "--head-lr-mult", type=float, default=1.0, help="learning rate multiplier for head during finetuning"
    )

    # pretraining type and backbone selection`
    group.add_argument("--vision-pretraining", action="store_true", help="flag to indicate vision pretraining")
    group.add_argument(
    ...
    group.add_argument(
        "--vision-backbone-type", type=str, default="vit", choices=["vit", "mit", "swin"], help="backbone types types"
    )
    group.add_argument(
        "--swin-backbone-type", type=str, default="tiny", choices=["tiny", "base", "h3"], help="pretraining objectives"
    )

    # inpainting arguments
    group.add_argument("--mask-type", type=str, default="random", choices=["random", "row"], help="mask types")
    ...
    group.add_argument("--dino-local-img-size", type=int, default=96, help="Image size for vision classification task")
    group.add_argument("--dino-local-crops-number", type=int, default=10, help="Number of local crops")
    group.add_argument("--dino-head-hidden-size", type=int, default=2048, help="Hidden dimension size in dino head")
    group.add_argument("--dino-bottleneck-size", type=int, default=256, help="Bottle neck dimension in dino head ")
    group.add_argument("--dino-freeze-last-layer", type=float, default=1, help="Freezing last layer weights")
    group.add_argument("--dino-norm-last-layer", action="store_true", help="Disable Norm in last layer.")
    group.add_argument("--dino-warmup-teacher-temp", type=float, default=0.04, help="warump teacher temperature")
    group.add_argument("--dino-teacher-temp", type=float, default=0.07, help="teacher temperature")
    group.add_argument(
        "--dino-warmup-teacher-temp-epochs", type=int, default=30, help="warmup teacher temperaure epochs"
    )
