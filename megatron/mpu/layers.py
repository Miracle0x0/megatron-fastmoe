    ...
    def __init__(self, num_embeddings, embedding_dim, init_method=init.xavier_normal_):
        ...
        if args.use_cpu_initialization:
            self.weight = Parameter(
                torch.empty(self.num_embeddings_per_partition, self.embedding_dim, dtype=args.params_dtype)
            )
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        if args.use_cpu_initialization:
            self.weight = Parameter(
                torch.empty(self.output_size_per_partition, self.input_size, dtype=args.params_dtype)
            )
        ...
        if bias:
            if args.use_cpu_initialization:
                ...
                self.bias = Parameter(
                    torch.empty(
                        self.output_size_per_partition, device=torch.cuda.current_device(), dtype=args.params_dtype
                    )
                )
...
class RowParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        if args.use_cpu_initialization:
            self.weight = Parameter(
                torch.empty(self.output_size, self.input_size_per_partition, dtype=args.params_dtype)
            )
            self.master_weight = _initialize_affine_weight_cpu(
                self.weight,
                self.output_size,
        ...
        if bias:
            if args.use_cpu_initialization:
                ...
                self.bias = Parameter(
                    torch.empty(self.output_size, device=torch.cuda.current_device(), dtype=args.params_dtype)
                )
