...
def get_checkpoint_name(checkpoints_path, iteration, release=False):
    ...
    return os.path.join(
        checkpoints_path,
        directory,
        "mp_rank_{:02d}_{:03d}".format(mpu.get_tensor_model_parallel_rank(), mpu.get_pipeline_model_parallel_rank()),
        "model_optim_rng.pt",
    )
...
def get_rng_state():
    ...
    rng_state_list = None
    if torch.distributed.is_initialized() and mpu.get_data_parallel_world_size() > 1 and args.data_parallel_random_init:
    ...
    if checkpoint_version < 2.0:
        ...
        for name, param in model.named_parameters():
            ...
            if name.endswith((".key_value.weight", ".key_value.bias")):
                ...
                param.data.copy_(fixed_param)
        print_rank_0(
            " succesfully fixed query-key-values ordering for" " checkpoint version {}".format(checkpoint_version)
        )
