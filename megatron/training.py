...
def pretrain(
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        all_data_iterators = [
            build_train_valid_test_data_iterators(train_valid_test_dataset_provider) for _ in range(len(model))
        ]
    ...
    if args.do_valid:
        ...
        evaluate_and_print_results(
            prefix, forward_step_func, valid_data_iterator, model, iteration, process_non_loss_data_func, False
        )
    ...
    if args.do_test:
        ...
        evaluate_and_print_results(
            prefix, forward_step_func, test_data_iterator, model, 0, process_non_loss_data_func, True
        )
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    if wrap_with_ddp:
        if args.DDP_impl == "torch":
            ...
            model = [
                torchDDP(model_module, device_ids=[i], output_device=i, process_group=mpu.get_data_parallel_group())
                for model_module in model
            ]
            ...
            model = [
                LocalDDP(
                    model_module, args.accumulate_allreduce_grads_in_fp32, args.use_contiguous_buffers_in_local_ddp
                )
                for model_module in model
            ]
...
def setup_model_and_optimizer(model_provider_func, model_type, no_wd_decay_cond=None, scale_lr_cond=None, lr_mult=1.0):
    ...
    optimizer = get_megatron_optimizer(unwrapped_model, no_wd_decay_cond, scale_lr_cond, lr_mult)
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        loss_dict, skipped_iter, grad_norm, num_zeros_in_grad = train_step(
            forward_step_func, train_data_iterator, model, optimizer, opt_param_scheduler
        )
        iteration += 1
...
def evaluate(forward_step_func, data_iterator, model, process_non_loss_data_func, verbose=False):
    """Evaluation."""
    args = get_args()
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            loss_dicts = forward_backward_func(
                forward_step_func, data_iterator, model, optimizer=None, timers=None, forward_only=True
            )
...
def evaluate_and_print_results(
    prefix, forward_step_func, data_iterator, model, iteration, process_non_loss_data_func, verbose=False
):
