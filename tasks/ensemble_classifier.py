...
def main():
    ...
    parser.add_argument(
        "--prediction-name", default="test_predictions.pt", help="name of predictions in checkpoint directories"
    )
    parser.add_argument("--calc-threshold", action="store_true", help="calculate threshold classification")
    parser.add_argument("--one-threshold", action="store_true", help="use on threshold for all subdatasets")
    parser.add_argument(
        "--threshold", nargs="+", default=None, type=float, help="user supplied threshold for classification"
    )
