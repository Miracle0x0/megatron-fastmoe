...
def get_tasks_args(parser):
    ...
    group.add_argument(
        "--pretrained-checkpoint", type=str, default=None, help="Pretrained checkpoint used for finetunning."
    )
    group.add_argument(
        "--keep-last", action="store_true", help="Keep the last batch (maybe incomplete) in" "the data loader"
    )
    group.add_argument(
        "--train-data", nargs="+", default=None, help="Whitespace separated paths or corpora names " "for training."
    )
    group.add_argument("--valid-data", nargs="*", default=None, help="path(s) to the validation data.")
    group.add_argument("--overlapping-eval", type=int, default=32, help="Sliding window for overlapping evaluation.")
    ...
    group.add_argument("--qa-data-test", type=str, default=None, help="Path to the QA dataset test file.")
    ...
    group.add_argument(
        "--faiss-topk-retrievals", type=int, default=100, help="Number of blocks to use as top-k during retrieval"
    )
    ...
    group.add_argument(
        "--train-with-neg", action="store_true", help="Whether to use negative examples during model " "training"
    )
    group.add_argument(
        "--train-hard-neg", type=int, default=0, help="Number of hard negative exmaples to use during " "training"
    )
