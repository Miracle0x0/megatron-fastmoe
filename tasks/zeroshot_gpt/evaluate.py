...
def get_model_provider(eval_metric):
    ...
    def model_provider(pre_process=True, post_process=True):
        ...
        model = GPTModel(
            num_tokentypes=0, parallel_output=parallel_output, pre_process=pre_process, post_process=post_process
        )
...
def forward_step(batch, model, eval_metric):
    ...
    if mpu.is_pipeline_last_stage():
        ...
        if eval_metric == "loss":
            losses = mpu.vocab_parallel_cross_entropy(output.contiguous().float(), labels.contiguous())
