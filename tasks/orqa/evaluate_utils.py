...
class ORQAEvaluator(object):
    ...
    def faiss_wrapper(self):
        ...
        if args.local_rank == 0:
            ...
            self.mips_index = FaissMIPSIndex(
                embed_size=self.embedding_size, embed_data=self.evidence_embedder_obj, use_gpu=self.faiss_use_gpu
            )
    ...
    def evaluate(self, qa_data, split):
        ...
        match_stats = calculate_matches(
            passages, reference_list, top_ids_and_scores, workers_num=args.num_workers, match_type=args.faiss_match
        )
